...
func outputTree(root *Node) []int {
    ...
    node_list := list.New()
    node_list.PushFront(root)

    for node_list.Len() > 0 {
        curLength := node_list.Len()
        for i := 0; i < curLength; i++ {
            node := node_list.Remove(node_list.Back()).(*Node)
            result = append(result, node.Value)
            if node.Left != nil {
                node_list.PushFront(node.Left)
            }
            if node.Right != nil {
                node_list.PushFront(node.Right)
            }
        }
    }
